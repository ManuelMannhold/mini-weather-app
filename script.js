...
async function getData() {
    ...
    if (inputValue.value === "") {
        ...
        let response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${inputValue.value}&appid=${API_KEY}&units=metric&lang=de`);
...
function displayWeather(weather) {
    ...
    let background = setBackground(weather);
    let weatherCard = document.getElementById('weather-card');
    weatherCard.innerHTML = '';

    weatherCard.innerHTML += displayWeatherTemplate(weatherName, weatherTemp, weatherHumidity, sunriseTime, sunsetTime, emoji, background);
}

function displayWeatherTemplate(weatherName, weatherTemp, weatherHumidity, sunriseTime, sunsetTime, emoji, background) {
    return `
...
function displayError() {
    ...
    return container.innerHTML =
        `<div class="card-error">Keine Daten vorhanden.</div>`;
}
...
function setBackground(city) {
    ...
    if(time > getSunsetTime(city)) {
        return 'linear-gradient(0deg, rgba(0,0,0,0.7) 24%, rgba(45,59,253,1) 100%);'
    } else {
        return 'linear-gradient(0deg, rgba(135,206,250,1) 0%, rgba(255,255,255,1) 100%)';
    }
}